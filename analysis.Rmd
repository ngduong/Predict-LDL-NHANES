...
Next we can look at a correlation matrix to see if the outcome is highly correlated with any of the predictors and the multicollinearity among them,
...
We can see that the 10-fold cross-validation seems to select elastic net with mixing percentage 0.2 as best model. However, the median cross-validated RMSE of elastic net and LASSO are very similar (35.93 vs. 35.94). The test RMSE on the held-out data was also very similar 1247.96 for LASSO and 1247.29 for elastic net.
...
The violin plot compares the models obtained from two different types of cross-validation (10-fold and Monte Carlo), as well as obtain the test error (using the test dataset). Although the differences are meager, both cross-validation methods pick the LASSO as the 
...
gam.m2 = gam(lbdldl~ridageyr + diq010 + s(lbxlypct, lbxnepct) + dr1talco + s(lbxhscrp) + s(lbxsgb) +s(lbxsgb, lbxhct) + bpq020 + s(lbxsapsi) + dbq700 + s(lbxsldsi)+ s(lbxsldsi), data = chol_train)
